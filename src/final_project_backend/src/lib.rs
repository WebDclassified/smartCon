...
fn get_proposal(key: u64) -> Option<Proposal> {
    PROPOSAL_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal, VirtualMemory::rc::Rc<|RefCell<Vec<u8>>>>>>| p.borrow().get(&key))
}

#[ic_cdk::query]
fn get_proposal_count() -> u64 {
    PROPOSAL_MAP.wit(|p: &RefCell<StableBTreeMap<u64, Proposal, VirtualMemory::rc::Rc<|RefCell<Vec<u8>>>>>>| p.borrow().len())
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal, VirtualMemory::rc::Rc<|RefCell<Vec<u8>>>>>>|{
...
fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    ...
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal, VirtualMemory::rc::Rc<|RefCell<Vec<u8>>>>>>|{
